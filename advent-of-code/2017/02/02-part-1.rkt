...
;; 8 + 4 + 6 = 18
(define test-matrix '((5 1 9 5) (7 5 3) (2 4 6 8)))

(define (check-sum acc xs)
  (cond ((not (pair? xs)) acc)
        (else (check-sum
               (+ acc
                  (difference-min-max-pair (min-max (car xs))))
               (cdr xs)))))

(define (min-max xs)
  (define (recur min max xs)
    (cond ((null? xs) (cons min max))
          ((< (car xs) min)
           (recur (car xs) max (cdr xs)))
          ((> (car xs) max)
           (recur min (car xs) (cdr xs)))
          (else (recur min max (cdr xs)))))
  (recur (car xs) (car xs) xs))

(define (difference-min-max-pair p)
  (- (cdr p) (car p)))

;; (check-sum 0 int-matrix)

(define fl (file->lines "./input-part-1.txt" 	#:line-mode 'linefeed))
...
;;;;
;; I am not using those, but it was a good practice
;;;;
(define (min xs)
  (define (recur m xs)
    (cond ((null? xs) m)
